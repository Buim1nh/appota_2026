...
export function CharacterLanding() {
  ...
  return (
    <div className="relative min-h-svh overflow-x-hidden">
      {/* Nền cố định toàn viewport — nhiều lớp gradient + blur tạo không gian tối fantasy */}
      <div
        className="pointer-events-none fixed inset-0 -z-10 bg-[#06080c]"
        aria-hidden
      />
      {/* Vùng sáng nhẹ phía dưới — “sàn” ấm */}
      <div
        className="pointer-events-none fixed inset-0 -z-10 bg-[radial-gradient(ellipse_120%_80%_at_50%_100%,rgba(30,22,12,0.9),transparent)]"
        aria-hidden
      />
      {/* Orb màu mờ + blur — chiều sâu */}
      <div
        className="pointer-events-none fixed inset-0 -z-10 bg-[radial-gradient(circle_at_20%_30%,rgba(120,80,40,0.15),transparent_45%),radial-gradient(circle_at_80%_70%,rgba(40,60,50,0.2),transparent_50%)] opacity-40 blur-3xl"
        aria-hidden
      />
      {/* Viền trên xanh lục nhạt → đen dưới */}
      <div
        className="pointer-events-none fixed inset-0 -z-10 bg-gradient-to-b from-emerald-950/20 via-transparent to-black/60"
        aria-hidden
      />

      {/* Tiêu đề giả + vùng phải: đã login thì chào + Đăng xuất; chưa thì link */}
      <header className="border-b border-zinc-900/80 bg-black px-4 py-3 sm:px-8">
